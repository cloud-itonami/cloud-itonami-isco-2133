(ns envpro.actor
  "EnvironmentalActor — the ISCO-08 2133 environmental protection actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one environmental operation request
  ...
  The unconditional invariant: the EnvironmentalAdvisor can never
  directly commit a record or dispatch an action the
  EnvironmentalProtectionGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [envpro.advisor :as advisor]
            [envpro.governor :as governor]
            [envpro.store :as store]))

(defn build-graph
  "Build a compiled EnvironmentalActor graph. `store` implements
  `envpro.store/Store`. `advisor` implements
  `envpro.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one environmental operation request to completion or interrupt.
